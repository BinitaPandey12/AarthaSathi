...
export default function BorrowerDashboard() {
  ...
  const acceptOffer = () => {
    setShowOffer(false);
    setPopup({ show: true, message: 'Offer accepted! Waiting for lender payment.' });
    setTimeout(() => {
  ...
  const handleLenderPayment = (loan) => {
    setPopup({ show: true, message: 'Lender payment received! Loan is now active.' });
    setTimeout(() => {
      setPopup({ show: false, message: '' });
      ...
      <div className="borrower-dashboard-root">
        ...
        <header className="borrower-header">
          ...
          <div className="borrower-welcome">
            ...
            <div className="borrower-desc">Here's your borrowing dashboard where you can request loans and manage your active loans.</div>
          </div>
        ...
        <section className="borrower-section">
          ...
          <div className="section-desc">These are loan offers matching your requirements.</div>
            ...
            <table className="borrower-table">
              ...
              <tbody>
                {lenderOffers.map((offer) => (
                  <tr key={offer.id}>
                    <td>
                      <div className="lender-info" style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
                        {/* SVG Profile Icon */}
                        <svg width="28" height="28" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg" style={{ display: 'inline-block' }}>
                          ...
                          <ellipse cx="12" cy="18" rx="6" ry="4" fill="#b6c4d6"/>
                        </svg>
                        <div>
                          <div className="lender-name">{offer.lender}</div>
                          <div className="lender-location">{offer.location}</div>
                        </div>
                      </div>
                    </td>
                    ...
                    <td>
                      <button className="view-offer-btn" onClick={() => openOffer(offer)}>
                        View Offer
                      </button>
                    </td>
                  </tr>
        ...
        <section className="borrower-section pending-section smart-pending-section">
          ...
          <div className="section-desc">Loans you've accepted, waiting for lender payment.</div>
          <div className="pending-loans-grid smart-pending-grid">
            {pendingLoans.length === 0 ? (
              <div className="pending-empty">No pending loans at the moment.</div>
            ) : (
              pendingLoans.map((loan) => (
                <div className="pending-loan-card smart-pending-card" key={loan.id}>
                  ...
                  <div className="loan-details">
                    <div>Amount <b>₹{loan.amount.toLocaleString()}</b></div>
                    <div>Interest <b>{loan.interest}%</b></div>
                    <div>Repays <b>{loan.repayment}</b></div>
                  </div>
                  ...
                  <div className="loan-actions">
                    <button className="make-offer-btn" onClick={() => handleLenderPayment(loan)}>Simulate Lender Payment</button>
                  </div>
                </div>
              ))
            )}
          ...
          <section className="borrower-section active-loans-section smart-active-section">
            ...
            <div className="active-loans-grid smart-active-grid">
              {activeLoans.map((loan) => (
                <div className="active-loan-card smart-active-card" key={loan.id}>
                  <div className="loan-header">
                    <div className="loan-lender">{loan.lender}</div>
                    <div className="loan-location">{loan.location}</div>
                  ...
                  <div className="loan-actions">
                    {loan.contract && <button className="contract-btn">View Contract</button>}
                  </div>
                </div>
              ))}
            </div>
          ...
          <section className="borrower-section create-loan-section smart-offer-section">
            ...
            <div className="create-loan-form">
              ...
              <input 
                ...
                onChange={(e) => setNewLoanForm({...newLoanForm, maxInterest: e.target.value})}
              />
              <input 
                type="text" 
                placeholder="Repayment Date (dd/mm/yyyy)" 
                value={newLoanForm.repaymentDate}
                onChange={(e) => setNewLoanForm({...newLoanForm, repaymentDate: e.target.value})}
              />
              <textarea 
                placeholder="Brief Description" 
                value={newLoanForm.description}
                onChange={(e) => setNewLoanForm({...newLoanForm, description: e.target.value})}
              />
          ...
          <div className="modal-overlay">
            <div className="modal-card">
              ...
              <div className="modal-content">
                <div><b>Lender:</b> {selectedOffer.lender}</div>
                <div><b>Location:</b> {selectedOffer.location}</div>
                <div><b>Amount:</b> ₹{selectedOffer.amount.toLocaleString()}</div>
                <div><b>Interest:</b> {selectedOffer.interest}%</div>
                <div><b>Repayment:</b> {selectedOffer.repayment}</div>
                <div><b>Trust Score:</b> {selectedOffer.trustScore}/10</div>
              </div>
              <div className="modal-actions">
                <button className="make-offer-btn" onClick={acceptOffer}>Accept Offer</button>
                <button className="view-contract-btn" onClick={closeOffer}>Cancel</button>
              </div>
            </div>
          </div>
        )}
        ...
        <section className="learn-section">
          ...
          <div className="learn-content">
            <div className="learn-item">
              ...
              <div className="learn-item-desc">A small portion of each loan goes into a safety pool to protect lenders in case of fraud. This ensures the platform remains safe for all users.</div>
            </div>
            <div className="learn-item">
              <div className="learn-item-title">Digital Contracts</div>
              <div className="learn-item-desc">All loans are backed by digital contracts that clearly outline terms, interest rates, and repayment schedules.</div>
            </div>
            <div className="learn-item">
              <div className="learn-item-title">Trust Scores</div>
              <div className="learn-item-desc">Your trust score is based on your borrowing history and repayment behavior. Higher scores help you get better loan terms.</div>
            </div>
            <div className="learn-item">
              <div className="learn-item-title">Escrow System</div>
              <div className="learn-item-desc">Funds are held securely in escrow until both parties agree on the loan terms, ensuring safe and transparent transactions.</div>
            </div>
          </div>
        </section>
      </div>
